...
def generate_cheapest_geometry(n_regions):
    # Generates the Dufek stability pin
    ...
    uo2.add_element('U', 1., enrichment=3.1)
    uo2.add_element('O', 2.)
    uo2.depletable = True
    uo2.temperature = 300.0
    uo2.volume = np.pi * r_fuel**2 * height / n_regions

    # Material 2: Clad
    clad_mat = openmc.Material(name='clad')
    clad_mat.set_density('g/cm3', 5.68)
    clad_mat.add_element('Zr', 1.)
    clad_mat.depletable = False
    clad_mat.temperature = 300.0
    clad_mat.volume = np.pi * (r_clad**2 - r_fuel**2) * height

    # Material 3: Cool
    cool_mat = openmc.Material(name='cool')
    cool_mat.set_density('g/cm3', 0.7)
    cool_mat.add_element('O', 1.)
    cool_mat.add_element('H', 2.)
    cool_mat.add_s_alpha_beta('c_H_in_H2O')
    cool_mat.depletable = False
    cool_mat.temperature = 300.0
    cool_mat.volume = pitch**2 * height - np.pi * r_clad**2 * height

    # Make surfaces

    # Bounding box
    left = openmc.XPlane(x0=-pitch/2, name='left')
    left.boundary_type = 'reflective'
    right = openmc.XPlane(x0=pitch/2, name='right')
    right.boundary_type = 'reflective'
    back = openmc.YPlane(y0=-pitch/2, name='back')
    back.boundary_type = 'reflective'
    forward = openmc.YPlane(y0=pitch/2, name='forward')
    forward.boundary_type = 'reflective'
    bottom = openmc.ZPlane(z0=-height/2, name='bottom')
    bottom.boundary_type = 'reflective'
    top = openmc.ZPlane(z0=height/2, name='top')
    top.boundary_type = 'reflective'

    # Cylinders
    f_out = openmc.ZCylinder(x0=0, y0=0, R=r_fuel, name='fuel OR')
    c_out = openmc.ZCylinder(x0=0, y0=0, R=r_clad, name='clad OR')

    # Cuts
    heights = np.linspace(-height/2, height/2, n_regions + 1)[1:-1]

    zsurf = [openmc.ZPlane(z0=height, name=str(i)) for i, height in enumerate(heights)]

    # Make cells

    # Clad
    clad = openmc.Cell(name='clad')
    clad.region = +f_out & -c_out & +bottom & -top
    clad.fill = clad_mat

    # Cool
    cool = openmc.Cell(name='cool')
    cool.region = +c_out & +bottom & -top & +left & -right & +back & -forward
    cool.fill = cool_mat

    # Fuel
    fuel = [openmc.Cell(name='fuel' + str(i)) for i in range(n_regions)]

    fuel[0].region = +bottom & -zsurf[0] & -f_out

    for i in range(n_regions-2):
        fuel[i+1].region = +zsurf[i] & -zsurf[i+1] & -f_out

    fuel[-1].region = +zsurf[-1] & -top & -f_out

    for i in range(n_regions):
        fuel[i].fill = uo2.clone()

    # Universe
    root = openmc.Universe(universe_id=0, name='root universe')

    root.add_cells([clad, cool])
    root.add_cells(fuel)

    geometry = openmc.Geometry(root)

    return geometry, [-pitch/2, -pitch/2, -height/2], [pitch/2, pitch/2, height/2]
