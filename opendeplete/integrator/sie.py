...
def sie_RR(operator, m=10, print_out=True):
    """ SIE with reaction rate relaxation.

    Mathematically equivalent to flux relaxation.

    Note, this is not coded in low memory form.
    ...
    for i, dt in enumerate(operator.settings.dt_vec):
        ...
        for j in range(m):
            ...
            for mat in range(n_mats):
                f = [operator.form_matrix(rates, mat) for rates in rates_array[1:]]

                f_bar = np.sum(f) / len(f)

                x_mat = CRAM48(f_bar, x[0][mat], dt)

                # Average together
                x_result_avg[mat] = copy.deepcopy(x_mat)
